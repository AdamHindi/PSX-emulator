...
bool DMA::irq() {
	...
	return irq_en && (force_irq || (channel_irq != 0));
}

uint32_t DMA::interrupt() {
	uint32_t r = 0;
	r |= uint32_t( irq_dummy & 0x03f);
...
void DMA::set_interrupt(uint32_t val) {
	...
	irq_en = ((val >> 23) & 1) != 0;
	auto ack = uint8_t((val >> 24) & 0x7f);
	channel_irq_flags &= ~ack;
}

uint32_t Channel::get_control() const{
	uint32_t r = 0;
...
void Channel::set_control(uint32_t val) {
	...
	chop_dma_sz = static_cast<uint8_t>((val >> 16) & 0x07);
	chop_cpu_sz = static_cast<uint8_t>((val >> 20) & 0x07);
